...
const notifications = [
  { id: "1", title: "Demande de votre enfant envoyée", subtitle: "Attestation d'inscription • Ahmed", status: "En attente", date: "2025-09-28 14:30", icon: "schedule" },
  { id: "2", title: "Bulletin de votre enfant validé", subtitle: "Bulletin S2 2025 • Sara", status: "Validé", date: "2025-09-27 10:15", icon: "check-circle" },
  { id: "3", title: "Demande rejetée pour votre enfant", subtitle: "Attestation de réussite • Youssef", status: "Rejeté", date: "2025-09-25 16:45", icon: "cancel" },
  { id: "4", title: "Demande rejetée pour votre enfant", subtitle: "Attestation de réussite • Lina", status: "Rejeté", date: "2025-09-25 16:45", icon: "cancel" },
]
...
export default function ParentNotificationsScreen() {
  ...
  const renderItem = ({ item, index }: { item: typeof notifications[0]; index: number }) => {
    ...
    return (
      <Animated.View
        className="flex-row items-start bg-white rounded-2xl p-4 mb-4 shadow-md"
...
<View className="px-6 py-4 flex-row items-center justify-between">
  <View>
    <Text className="text-xl font-inter-bold text-sky-800">Notifications Des Enfants</Text>
    <Text className="text-xs text-slate-500">Suivi des demandes et documents</Text>
  </View>
      ...
      <View className="flex-row px-4 mb-4 justify-between">
        {["Tous", "En attente", "Validé", "Rejeté"].map((status) => {
          ...
          return (
            ...
            >
              <Text className={`${isActive ? "text-white" : "text-slate-600"} font-inter`}>
                {status}
              </Text>
            </Pressable>
          )
